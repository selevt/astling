<script lang="ts">
	...
	let result: { deleted: string[]; failed: Array<{ branch: string; error: string }> } | null =
		$state(null);
...
<Dialog bind:open title="Merged Branch Cleanup">
	{#if phase === 'scanning'}
		<div class="scanning">
			...
			<div class="branch-list">
				<label class="branch-row toggle-all">
					...
					<span class="toggle-all-label">
						{allSelected ? 'Deselect all' : 'Select all'} ({branches.length} branch{branches.length ===
						1
							? ''
							: 'es'})
					</span>
	...
	{:else if phase === 'done'}
		<div class="done-summary">
			{#if result}
				<p class="success-msg">
					{result.deleted.length} deleted{result.failed.length > 0
